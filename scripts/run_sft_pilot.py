"""Pilot SFT: so sanh Qwen2.5-7B vs Qwen3.6-27B tren cung config toi thieu.
...
CONFIG = {
    ...
    "result_dir": f"/kaggle/working/run_{RUN_TAG}/results",
...
logger = Logger(f"{CONFIG['log_dir']}/pilot.log")
logger.log(f"BASE_MODEL={BASE_MODEL} RUN_TAG={RUN_TAG}")
...
# Qwen3.6 (kien truc phat hanh 04/2026) doi hoi transformers/torch moi, ma
# torch moi da bo ho tro bien dich cho sm_60 (P100). Chan som thay vi de
# loi mo ho luc load model.
if props.major < 7:
    logger.log(f"FATAL: GPU compute capability {cap} (< 7.0, i.e. P100 hoac cu hon) "
               f"khong tuong thich voi transformers/torch ban moi can cho Qwen3.6. "
               f"Can T4 (sm_75) tro len. Dung lai pilot nay.")
    sys.exit(1)
...
# unsloth's loader thuong nuot loi that va bao mot thong bao chung chung
# "Can't load the configuration" — probe truoc de thay ro nguyen nhan that
# (repo khong ton tai / architecture chua duoc ho tro / auth).
try:
...
def generate_batch(records):
    prompts = [make_prefix(r) for r in records]
    inputs = tokenizer(
...
for batch_start in range(0, len(test_data), BATCH_SIZE):
    ...
    logger2.log(f"[{done:>3}/{len(test_data)}] F1 so far: {f1_now:.4f}")
...
print("=" * 55)
print(f"\n>> [{RUN_TAG}] BASE_MODEL={BASE_MODEL} Overall F1: {f:.4f}")
...
out_path = f"{CONFIG['result_dir']}/eval_pilot_{RUN_TAG}.json"
